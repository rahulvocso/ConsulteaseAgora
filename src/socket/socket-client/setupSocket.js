...
// const socket = io(config.socketServerUrl, { autoConnect: false, 'Access-Control-Allow-Origin': '*' }); //{ transports: ["websocket"] }

function setupSocket(consulteaseUserProfileData) {
  if(!Utils.socket){
    Utils.socket = io('http://192.168.1.33:3006', // io(config.socketServerUrl)
      { 
    ...
    Utils.socket.on('connect', () => {
      console.log('Socket connected - 1st time', socket.id);
